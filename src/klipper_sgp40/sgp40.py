...
class SGP40:
    ...
    def _init_sgp40(self):
        ...
        self._write_command(SGP40_CMD["SOFT_RESET"], delay=0.001)

        # Get the serial number to verify communication
        serial = self._write_command(SGP40_CMD["GET_SERIAL"], read_len=6, delay=0.001)
        if serial is None:
            logging.error(
                self._log_message(
        ...
        self_test_result = self._write_command(
            SGP40_CMD["SELF_TEST"], read_len=3, delay=0.32
        )
    ...
    def _write_command(self, command, delay=0.0, read_len=0):
        ...
        if "response" not in params or len(params["response"]) != reply_len:
            logging.error(self._log_message("Invalid response length from " "sensor"))
            return None

        response = bytearray(params["response"])
        data = [
            unpack_from(">H", response, i)[0]
            for i in range(0, reply_len, SGP40_WORD_LEN)
        ]
